...
* [8. TCP 建立可以两次握手吗? 为什么?](#8-tcp-建立可以两次握手吗-为什么)
* [9. TCP 可以采用四次握手吗? 为什么?](#9-tcp-可以采用四次握手吗-为什么)
* [10. 什么是四次挥手?](#10-什么是四次挥手)
* [参考资料](#参考资料)
...
`Client` 发出的第一次连接请求时因网络原因阻塞了，很久很久，以致延误到链接释放之后才到达 `Server`（该报文段已失效）

但 `Server` 收到此报文段后，误认为 `Client` 发来的新的连接请求，于是向 `Client` 发出确认报文段，同意建立连接（同时请求与 `Client` 连接）



假如 `TCP` 连接是两次握手：

在上述场景，只要 `Server` 发出确认，新的连接就已经建立

由于现在 `Client` 并没有发出建立连接的请求，因此不会理睬 `Server` 的确认（不会向 `Server` 发送数据），造成资源浪费	



而采用三次握手：

在上述场景，`Client` 不会向 `Server` 的确认发送确认（因此没有建立新连接）
