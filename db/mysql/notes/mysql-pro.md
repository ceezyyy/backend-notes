...
* [8. 索引优化实战](#8-索引优化实战)
   ...
   * [8.2 初始化](#82-初始化)
   * [8.3 全值匹配 恭喜发财](#83-全值匹配-恭喜发财)
   * [8.4 最左匹配原则](#84-最左匹配原则)
   * [8.5 不在索引列上做操作（计算/函数/类型转换）](#85-不在索引列上做操作计算函数类型转换)
   * [8.6 存储引擎不能使用索引中范围条件右边的列](#86-存储引擎不能使用索引中范围条件右边的列)
   * [8.7 尽量使用覆盖索引](#87-尽量使用覆盖索引)
   * [8.8 使用 !=, &lt;, &gt;, is null, is not null 索引会失效](#88-使用----is-null-is-not-null-索引会失效)
   * [8.9 like 以通配符开头, 索引会失效](#89-like-以通配符开头-索引会失效)
   * [8.10 字符串不加单引号, 索引会失效](#810-字符串不加单引号-索引会失效)
   * [8.11 少用 or, 用其连接时索引会失效](#811-少用-or-用其连接时索引会失效)
...
### 8.5 不在索引列上做操作（计算/函数/类型转换）



### 8.6 存储引擎不能使用索引中范围条件右边的列
...
### 8.8 使用 !=, <, >, is null, is not null 索引会失效
